<div>
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-form-section submit="addTeamMember">
                <x-slot name="title">
                    {{ __('Thêm thành viên nhóm') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Thêm một thành viên nhóm mới vào nhóm của bạn, cho phép họ cộng tác với bạn.') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600">
                            {{ __('Vui lòng cung cấp địa chỉ email của người bạn muốn thêm vào nhóm này.') }}
                        </div>
                    </div>
    ...
    @if ($team->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('Lời mời nhóm đang chờ xử lý') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Những người này đã được mời vào nhóm của bạn và đã được gửi email mời. Họ có thể tham gia nhóm bằng cách chấp nhận lời mời qua email.') }}
                </x-slot>
    ...
    @if ($team->users->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('Thành viên của nhóm') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Tất cả những người là một phần của đội này.') }}
                </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingLeavingTeam">
        ...
        <x-slot name="content">
            {{ __('Bạn có chắc chắn muốn rời nhóm này không?') }}
        </x-slot>
    ...
    <x-jet-confirmation-modal wire:model="confirmingTeamMemberRemoval">
        <x-slot name="title">
            {{ __('Xóa thành viên nhóm') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Bạn có chắc chắn muốn xóa người này khỏi nhóm không?') }}
        </x-slot>
